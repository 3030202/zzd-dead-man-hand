...
func listActions(ctx context.Context, cmd *cli.Command) error {
	...
	if err := json.Unmarshal(body, &data); err != nil {
		...
		_, printErr := fmt.Fprintf(outputWriter, "%s", body)
		return printErr
	}

	indented, err := json.MarshalIndent(data, "", "  ")
	if err != nil {
		// Should not happen if Unmarshal worked, but fallback just in case
		_, err := fmt.Fprintf(outputWriter, "%s", body)
		return err
	}

	_, err = fmt.Fprintln(outputWriter, string(indented))
	return err
}
